...
<p class="note"><b>Black pepper is the top intercrop at every point</b> in the temperature sweep — its lead is robust to the uncertain offset, so it is actionable now. <b>Nutmeg leads only at the cool end</b> and falls away as temperature rises (Anaikadu is at the edge of its flowering optimum), so its viability is conditional on the LOW-confidence offset — exactly what on-plot sensing would resolve.</p>
</div></div></section>
...
// recommendation
$('#recbox').innerHTML="<b>For Anaikadu:</b> plant <b>coconut as the overstorey with black pepper as the intercrop</b> for resilient annual cash — pepper clears the 8% hurdle (NPV ≈ "+inr(DATA.finance.find(f=>f.system.indexOf('Pepper')>=0).npv)+"/acre, IRR ≈ "+(DATA.finance.find(f=>f.system.indexOf('Pepper')>=0).irr*100).toFixed(0)+"%) and is the <b>top intercrop across the entire plausible temperature band</b>, with the lowest probability of loss. <b>Nutmeg is conditional</b>: it sits at the edge of its flowering optimum here and only wins at the cool end of the uncertain offset — viable if on-plot sensing confirms a cooler microclimate, risky otherwise. Add a <b>teak or mahogany block / boundary</b> as long-horizon capital and windbreak (high modelled return but a 15–18 yr lock-up, LOW-confidence prices). Avoid banana under mature coconut (heat- and shade-limited here). One season of on-plot sensing would resolve the nutmeg call and sharpen the rest.";
</script>
</body></html>"""
