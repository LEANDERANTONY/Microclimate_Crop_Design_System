...
![tests](https://img.shields.io/badge/tests-35%20passing-3f8f4f) ![python](https://img.shields.io/badge/python-3.11-4a6b3a) ![license](https://img.shields.io/badge/license-MIT-7a5a2e) ![status](https://img.shields.io/badge/pipeline-6%20layers%20built-2f3a28)
...
tests/              35 tests
papers/             modular three-paper split (paper1_microclimate, paper2_suitability, paper3_economics) over the shared core
docs/               architecture, modeling_blueprint, economics_layer, data_acquisition, external_validation_datasets, ADRs 001–015
reports/            results.json, loso/loco/benchmark/mondrian metrics, anaikadu_preprint.html, economics_qa, sourced inputs, catalogs
figures/            12 figures (regenerated by make_figures.py + make_paper_figures.py)
```
...
- Reanalysis can't resolve the village from the town (shared ~31 km ERA5 pixel) — an **on-plot logger (year 1)** remains the definitive fix, but be precise about what it buys (ADR-018): used as *calibration* data, few-shot conformal recalibration makes the intervals **honest by widening them**, not narrower; only folding a local source into **training** can genuinely narrow them. Do not expect a season of logger data to "collapse" the uncertainty on its own.
- **No passing independent external validation yet.** Within-climate skill above is leave-one-site-out on the *training* corpus. The first pre-registered external test (humid-tropical cocoa agroforestry) was run and scored once, and returned a **null result on methodological grounds** — the source publishes one station coordinate, so the canopy→offset mapping cannot vary across its plots, and our ERA5 free-air ambient reference is unrepresentative at that high-relief valley site. The independent Mediterranean leg is blocked pending outstanding datasets. Register: [`docs/external_validation_datasets.md`](docs/external_validation_datasets.md).
- The **ambient-reference convention has a stated applicability limit** (ADR-017): ERA5 free-air is valid over low-relief terrain only, and candidate validation sites are screened before use. Anaikadu is unaffected (8 m from its cell mean, 43 m relief), so the results above stand.

Treat **comparisons** (design A vs B, crop ranking, dry vs wet timing) as reliable and absolute numbers as indicative-with-stated-uncertainty. Every modelling decision is recorded in [`docs/architectural_decision_records/`](docs/architectural_decision_records/) (ADR-001–017).

---

## Status

All six layers built, validated, and runnable (35 tests). Real data integrated across two macroclimates + an open-canopy regime. Transfer validated with skill scores + leave-one-climate-out; a physics-prior hybrid tested (ADR-012); design→feature mapping grounded on real TN satellite values (ADR-013); a six-family model benchmark packaged and tested (`models_benchmark.py`); work organised into a modular three-paper program (ADR-015, `papers/`). **External validation is in progress and has not yet produced a passing result** — the first pre-registered test returned a methodological null and the Mediterranean leg is blocked (see *Honest limitations*, ADR-016/017). Outstanding (manual / data): independent within-climate validation sets and SoilTemp / tropical-understory data (to close the climate gap), CEDA 3-yr prices, and the user's own plot sensor. *Research output — model results with stated uncertainty, not guarantees.*
