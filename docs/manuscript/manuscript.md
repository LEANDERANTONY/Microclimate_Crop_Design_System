...
*Suitability and its robustness.* Scoring candidate intercrops under coconut, nutmeg and black pepper lead; all candidates are temperature-limited at this hot site. Sweeping the *uncertain* temperature offset across its full plausible band changes absolute viability but **not the shortlist** (nutmeg/pepper/banana ordering is stable) — so the decision of *which* crop is actionable now; the pending data sharpens only *how well*.

*Economics, finance, risk.* Over 25 years at an 8 % real hurdle: coconut + **black pepper** clears it with **NPV ≈ ₹298k/acre, IRR ≈ 22 %, payback 8 yr** and the lowest probability of loss among the intercrops (**P(loss) ≈ 17 %**); coconut + nutmeg has a higher mean NPV (≈ ₹396k) but a fatter tail (P(loss) ≈ 35 %, bimodal — hot draws fail the crop, cool draws pay well); coconut alone is positive but modest (≈ ₹129k, P(loss) ≈ 3 %); banana under mature coconut is uneconomic (heat- and shade-limited). Timber overstoreys show high *annualised* return but concentrate all risk in a single distant harvest at 15–18 yr and rest on LOW-confidence farm-gate prices.
...
2. **Interval calibration out-of-climate.** Conformal coverage holds within climate but collapses across climates; per-climate (Mondrian) conformal calibration is the natural next step.
...
*Figures (in `figures/`): Fig. 1 cross-climate transfer; Fig. 2 microclimate by overstorey; Fig. 3 suitability + sensitivity; Fig. 4 economics; Fig. 5 Monte-Carlo NPV; Fig. 6 cash-flow timing. An interactive version of all results is in `reports/anaikadu_preprint.html`.*
